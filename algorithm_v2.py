...
from common_utils import (
    R_earth, GAMMA_SCALE, NumpyEncoder,
...
# Fixed ground stations (all benchmark configurations share this)
gs_lat_lon_20 = [
...
if __name__ == "__main__":
    ...
    disp_file = os.path.join(SCRIPT_DIR, 'nonlocal_dispersion_report.json')
    if os.path.exists(disp_file):
        ...
        # Saturation model is HYPOTHETICAL — only gamma=6.0 validated.
        n_baseline = 91.6   # [HYPOTHETICAL]
        n_grid_max = 123.1  # [HYPOTHETICAL]
        gamma_char = 0.573  # [HYPOTHETICAL]
        C0 = 30.1556
        gamma_c_06 = 0.444
    
    print(f"  n_baseline   = {n_baseline:.1f} cores (gamma=0, HYPOTHETICAL)")
    print(f"  n_grid_max   = {n_grid_max:.1f} cores (gamma→∞, HYPOTHETICAL)")
    print(f"  gamma_char   = {gamma_char:.3f}")
    print(f"  C0           = {C0:.4f}")
    print(f"  gamma_c(0.6) = {gamma_c_06:.4f}")
    print(f"  n_cores is INDEPENDENT of N (Round 15 fix)")
    print(f"  C++ validated: n_cores ≈ 92.3 at gamma=6.0 (both N=400 and N=1000)")
    
    # Update common_utils with loaded parameters (so shared functions use correct values)
    common_utils.n_baseline = n_baseline
    common_utils.n_grid_max = n_grid_max
    common_utils.gamma_char = gamma_char
    common_utils.C0 = C0
    common_utils.gamma_c_06 = gamma_c_06
    
    # =====================================================================
    # Part B: Core Count Prediction (imported from common_utils)
    # =====================================================================
    ...
    # Test predictions (using functions imported from common_utils)
    print(f"\n  Core count predictions (n_cores INDEPENDENT of N, Round 15):")
    print(f"  n_cores is absolute, not fraction of N. Only gamma=6.0 validated by C++.")
    for frac in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30]:
        g_req = required_gamma_for_core_fraction(frac)
        n_pred = predict_cores(g_req, N=400)
        print(f"    target={frac*400:.0f} cores → γ={g_req:.3f} → n_cores={n_pred:.0f} [HYPOTHETICAL]")
    ...
    # (generate_network uses internal RandomState, no need for global seed)
...
# benchmark_greedy, benchmark_roundrobin, benchmark_nearest3
# are imported from common_utils
    ...
    for j in range(M):
        ...
        for c_idx, c_w in zip(core_idxs, core_weights):
            ...
            if len(core_sats) > 0:
                ...
                total_dist += np.min(sat_dists) * c_w
...
def capacity_mbps(d_km, f_ghz=30, bw_mhz=200, P_tx_dbm=33, G_antenna_dbi=25,
    ...
    Args:
        ...
        link_type: 'isl' (inter-satellite, vacuum) or 'sat_ground' (with atmospheric attenuation)
    """
    fspl_db = 32.4 + 20*np.log10(f_ghz) + 20*np.log10(max(d_km, 1.0))
    # Atmospheric attenuation only applies to satellite-to-ground links (troposphere path)
    atten_db = atmospheric_attenuation_db(d_km, f_ghz, weather) if link_type == 'sat_ground' else 0.0
...
def estimate_protocol_overhead(N, n_cores, update_interval_s=10):
    ...
    NOTE: The manuscript (§4.11) uses a more conservative byte-based formula:
    (n_cores × k + N) × 64 bytes/cycle. This function uses bit-level estimates
    for finer granularity, yielding comparable order-of-magnitude results.
    Both confirm overhead << 0.1% engineering feasibility threshold.
    """
    phi_broadcast = N * 4 * 8 / 1000  # kbps: 4-byte float per satellite (= 32 bits)
    # Core mesh: each core exchanges with ~6 neighboring cores (not all-to-all)
    k_neighbors = min(6, max(n_cores - 1, 0))
    core_mesh = n_cores * k_neighbors * 64 / 1000  # 64 bits (= 8 bytes) per core pair
    routing = n_cores * (N / max(n_cores, 1)) * 32 / 1000  # 32 bits (= 4 bytes) per satellite
...
def generate_gs_distributions(seed=42):
    ...
    northern = [(rng.uniform(30, 60), rng.uniform(-180, 180)) for _ in range(16)]
    northern += [(rng.uniform(-60, 30), rng.uniform(-180, 180)) for _ in range(4)]
    # 3. Oceanic (Pacific/Atlantic/Indian corridors) — 20 GS
    oceanic = [(rng.uniform(-5, 45), rng.uniform(120, 240)) for _ in range(7)]
    oceanic += [(rng.uniform(20, 55), rng.uniform(-80, -10)) for _ in range(7)]
    oceanic += [(rng.uniform(-30, 10), rng.uniform(30, 100)) for _ in range(6)]
    # 4. Southern hemisphere concentrated — 20 GS (added for fair CV comparison)
    southern = [(rng.uniform(-60, -20), rng.uniform(-180, 180)) for _ in range(16)]
    southern += [(rng.uniform(-20, 60), rng.uniform(-180, 180)) for _ in range(4)]
    # 5. Sparse (10 stations)
    sparse = [base_gs[i] for i in range(0, 20, 2)]
    # 6. Dense (50 random stations)
    dense = [(rng.uniform(-60, 60), rng.uniform(-180, 180)) for _ in range(50)]
...
def generate_network_from_config(config, seed=42):
    """Generate satellite positions from extended constellation config.
    
    Args:
        config: dict with 'heights' and 'sats_per_layer'.
...
# =====================================================================
# Part D8: Core detection helper (imported from common_utils)
# _detect_cores is imported from common_utils
# =====================================================================


def benchmark_cbdp_v3_demand_weighted(sat_pos, gs_pos, gs_demand, gamma=6.0, alpha=0.3, k_cores=3):
    """
    CBDP v3 with demand-weighted φ-field: core detection is biased by GS demand.
    Computes demand-weighted sat_weight, then delegates to _detect_cores and benchmark_cbdp_v3.
    """
    N = len(sat_pos)
    M = len(gs_pos)

    # Build demand-weighted satellite weights
    ...
    # Delegate core detection to shared _detect_cores with demand weights
    demand_cache = _detect_cores(sat_pos, gamma, N, sat_weight=sat_weight)

    # Phase 2+3: Delegate to shared CBDP v3
    return benchmark_cbdp_v3(sat_pos, gs_pos, gs_demand, gamma=gamma, alpha=alpha,
                             k_cores=k_cores, core_cache=demand_cache)
...
if __name__ == "__main__":
    ...
    for cfg in constellation_tests:
        ...
        # Save medium-scale (N=500) raw results for Part N/P queue/throughput analysis
        if cfg['N'] == 500:
            r_greedy_N500 = r_greedy
            r_nearest3_N500 = r_nearest3
            r_cbdp_N500 = r_cbdp
            r_cbdp_v3_N500 = r_cbdp_v3
    ...
    for run in range(N_RUNS):
        sat_pos = generate_network(sig_N, sig_layers, seed=42 + run)
    ...
    # GAMMA_SCALE Calibration: iterate common_utils.GAMMA_SCALE directly
    # (NOT algorithm_v2's imported binding — _detect_cores reads common_utils.GAMMA_SCALE)
    original_scale = common_utils.GAMMA_SCALE
    
    for scale in scales:
        common_utils.GAMMA_SCALE = scale
    ...
    common_utils.GAMMA_SCALE = original_scale
    ...
    for t in range(24):
        ...
        r_t = benchmark_cbdp_v3_demand_weighted(sat_pos_tv, gs_positions, demands_t, gamma=gamma_t)
    ...
    # Run CBDP v3 and PDE Direct with INDEPENDENT core detection
    # (previously shared core_cache, which artificially inflated cosine similarity)
    r_cbdp_bridge = benchmark_cbdp_v3(sat_pos_bridge, gs_positions, gs_demands, gamma=gamma_opt)
    r_pde = benchmark_pde_direct(sat_pos_bridge, gs_positions, gs_demands, gamma=gamma_opt)  # independent core detection
    ...
    # Throughput for each algorithm at medium-scale (N=500)
    for algo_name, r_algo in [('Greedy', r_greedy_N500), ('Nearest-3', r_nearest3_N500),
                                ('CBDP v2', r_cbdp_N500), ('CBDP v3', r_cbdp_v3_N500)]:
    ...
    for N_test, n_c in [(66, results[0]['n_cores_actual']), (500, results[2]['n_cores_actual']),
        ...
        print(f"    N={N_test:>5}, n_cores={n_c:>4}: overhead={overhead:.1f}kbps "
              f"({overhead/(link_cap*1000)*100:.4f}% of 2000km link capacity)")
    ...
    # Compute queue-aware metrics for the medium-scale (N=500) benchmark results
    ...
    output = {
        ...
        "throughput_and_overhead": {
            ...
            'protocol_overhead_kbps': [{
                ...
                'pct_of_link_capacity': float(estimate_protocol_overhead(N_t, n_c) / (capacity_mbps(2000) * 1000) * 100)}
    ...
    with open(os.path.join(SCRIPT_DIR, "algorithm_v2_report.json"), 'w', encoding='utf-8') as f:
    ...
    5. CBDP v3 grid resolution scales with satellite density
       ...
       NEW: grid_res = sqrt(N*5), ~N/grid_res^3 sats per cell

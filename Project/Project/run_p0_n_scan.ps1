# P0: n_scan — N scaling scan (Round 31)
# Determines true n_cores vs N scaling
$n_values = @(200, 400, 600, 800, 1500, 2000)
$gamma = 6.0
$beta = 0.6
$grid_res = 40
$duration = 0.5
...
Write-Host "================================================"
Write-Host "P0: n_scan (N scaling)"
Write-Host "gamma=6.0, beta=0.6, grid=40"
...
foreach ($n in $n_values) {
    $output_name = "multilayer_results_n_scan_N$n.json"
    ...
    if (Test-Path $default_output) {
        ...
        $msg = "  [FAIL] N=" + $n + " : no output file generated"
        Write-Host $msg
        $failed++
    }
}
